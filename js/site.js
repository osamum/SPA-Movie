...
let baseContainer, movieListData, currentMovieData, display = $id('display');
...
function deleteListIem(index) {
    ...
    deleteButton.onclick = async () => {
        const response = await fetch(`${MOVIEAPP_API}/${currentMovieData.id}`, {
            method: "DELETE",
        });
        showListView();
...
async function postData(id) {
    ...
    try {
        if (id) {
            data.id = id
            const response = await fetch(`${MOVIEAPP_API}/${id}`, {
                method: "PUT",
                headers: {
